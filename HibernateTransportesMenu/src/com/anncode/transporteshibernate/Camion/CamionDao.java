...
public class CamionDao {
	...
	public List<Camion> obtenerRegistrosParametros(HashMap<String, Object> parametros, Session session){
		...
		try {
			...
			for (Camion ca : camiones){
				System.out.println("id"+"     "+"Matrícula"+"     "+"Modelo");
				System.out.println(ca.getId()+"      "+ca.getMatricula()+"       "+ca.getModelo());
			}
			session.getTransaction().commit();
	...
	public int eliminarParametros(HashMap<String, Object> parametros, Session session){
		int row_afectados = 0;
		
		String hql = "DELETE FROM Camion WHERE ";
		try {
			String parametrosQuery = "";
			int i = 1;
			for (Map.Entry<String, Object> parametro: parametros.entrySet()){
			...
			// TODO: handle exception
			System.err.println("****ERROR AL ELIMINAR LOS REGISTROS POR PARAMETRO****");
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();	
